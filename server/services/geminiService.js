...
const EMISSION_FACTORS = {
  ...
  'natural gas': 0.0020, // tCO2e per m3 / unit
  'gas': 0.0020,
...
function calculateFallbackAnalysis(onboardingData) {
  ...
  waste.forEach((item) => {
    ...
    const emissions = Math.round(qty * factor);
    totalEmissions += emissions;
    sourcesMap.set(name + ' (' + item.disposalMethod + ')', (sourcesMap.get(name) || 0) + emissions);
  });
  ...
  const recommendations = [
    ...
    },
    {
      ...
      payback: '1.5 years',
      priority: 'HIGH',
      description: 'Capture flue gas and process waste thermal energy to pre-heat primary operations, reducing raw energy demand.',
    },
    {
      id: 3,
      ...
      payback: '1.2 years',
      priority: 'MEDIUM',
      description: 'Implement automated scrap sorting and scrap re-feeding directly back into production cycles.',
    },
  ];
  ...
  const wasteReuseMatches = (waste.length > 0 ? waste : [
    { wasteType: 'Process Solid Waste', disposalMethod: 'Landfill' },
    { wasteType: 'Metal & Industrial Scrap', disposalMethod: 'Landfill' },
    { wasteType: 'Flue Gas & Thermal Exhaust', disposalMethod: 'Incineration' }
  ]).map((wItem, idx) => {
    const wName = wItem.wasteType || 'Industrial Slag / Scrap';
    const lowerW = wName.toLowerCase();
    
    if (lowerW.includes('metal') || lowerW.includes('steel') || lowerW.includes('scrap')) {
      return {
        id: idx + 1,
        wasteType: wName,
        reusabilityPotential: 'High (Excellent Market Value)',
        rawMaterialSubstitute: 'Feedstock Ingot & Re-melting Scrap',
        targetIndustries: 'Foundries, Steel Rolling Mills, Metallurgy Plants',
        estimatedByproductValue: '₹22,000 - ₹35,000 / ton',
        diversionStrategy: 'Briquette and segregate high-purity scrap to sell directly to electric arc furnace foundries as secondary raw feedstock, diverting 95%+ from landfills.',
      };
    } else if (lowerW.includes('ash') || lowerW.includes('slag') || lowerW.includes('solid')) {
      return {
        id: idx + 1,
        wasteType: wName,
        reusabilityPotential: 'High (Industrial Aggregates)',
        rawMaterialSubstitute: 'Supplementary Cementitious Feedstock & Road Base',
        targetIndustries: 'Cement Manufacturers, Concrete Block Plants, Road Construction',
        estimatedByproductValue: '₹1,500 - ₹3,000 / ton',
        diversionStrategy: 'Supply non-hazardous solid residue to local cement clinker manufacturers as a calcined clay/silica substitute for eco-friendly concrete production.',
      };
    } else {
      return {
        id: idx + 1,
        wasteType: wName,
        reusabilityPotential: 'Medium to High (Circular Byproduct)',
        rawMaterialSubstitute: 'Refined Recycled Polymer & Refractory Feedstock',
        targetIndustries: 'Recycled Packaging Plants, Thermal Power Auxiliary, Fertilizer Manufacturing',
        estimatedByproductValue: '₹8,000 - ₹15,000 / ton',
        diversionStrategy: 'Establish B2B circular byproduct exchange agreements with regional material recyclers to convert process waste into industrial packaging filler or secondary fuel.',
      };
    }
  });
...
async function analyzeCarbonFootprint(onboardingData) {
  ...
  if (!apiKey) {
    console.log('No GEMINI_API_KEY set in server env. Using fallback IPCC calculation engine.');
...
5. Generate an array ("wasteReuseMatches") identifying how generated waste streams are reusable, what raw material substitute they act as, target purchasing industries (e.g. Cement, Metallurgy, Packaging), byproduct market value (₹/ton), and diversion strategy.
...
Respond ONLY with JSON using this exact structure (no extra keys, no markdown codeblocks):
{
  ...
  "wasteReuseMatches": [
    {
      "id": 1,
      "wasteType": "Process Solid Scrap",
      "reusabilityPotential": "High (Excellent Market Value)",
      "rawMaterialSubstitute": "Raw Feedstock Ingot & Scrap Re-melting",
      "targetIndustries": "Metallurgy Plants, Foundries, Rolling Mills",
      "estimatedByproductValue": "₹25,000 / ton",
      "diversionStrategy": "Segregate and briquette production scrap to sell directly to electric arc foundries as raw material feedstock."
    }
  ]
}
`;
  ...
  for (const model of modelsToTry) {
    try {
      ...
      if (!response.ok) {
        console.warn(`Gemini API endpoint ${model} returned status ${response.status}. Trying next model...`);
        continue;
      }
  ...
  console.log('All Gemini API models failed or unreachable. Using fallback IPCC engine.');
  return calculateFallbackAnalysis(onboardingData);
}
