	...
	const chartData = useMemo(() => {
		...
		for (let year = 0; year <= years; year++) {
			if (year === 0) {
				...
				const inflatedSavings =
					totalAnnualSavings * Math.pow(1 + inflationRate / 100, year - 1);
			...
			<div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
				<div>
					...
					<p className="text-gray-500 text-sm mt-1">
						Financial impact & interactive projections for circular
						interventions
					</p>
				</div>
			</div>
			...
			{loading ? (
				...
				<div className="space-y-6">
					...
					<div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm flex flex-col xl:flex-row gap-8 border-t-4 border-t-emerald-400">
						...
						<div className="w-full xl:w-1/3 space-y-6">
							<h3 className="font-bold text-lg text-gray-900 flex items-center gap-2 border-b border-gray-100 pb-2">
								<Settings2 size={20} className="text-emerald-500" /> Prediction
								Variables
							</h3>
							...
							<div>
								...
								<p className="text-[10px] text-gray-400 mt-1">
									Expected annual increase in energy & raw material prices.
								</p>
							</div>
							...
							<div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 mt-4">
								...
								<div className="text-2xl font-extrabold text-emerald-700">
									...
									{formatINR(chartData[chartData.length - 1]?.netROI || 0)}
								</div>
							</div>
						</div>
						...
						<div className="w-full xl:w-2/3 h-[350px]">
							...
							<ResponsiveContainer width="100%" height="100%">
								...
								>
									...
									<Area
										type="monotone"
										name="Net ROI (Cumulative Savings - Cost)"
										dataKey="netROI"
										stroke="#10b981"
										strokeWidth={3}
										fillOpacity={1}
										fill="url(#colorNetROI)"
									/>
									<Line
										type="stepAfter"
										name="Initial Investment Cap"
										dataKey="cost"
										stroke="#f43f5e"
										strokeWidth={2}
										strokeDasharray="5 5"
					...
					<h3 className="font-bold text-xl text-gray-900 mt-8 mb-4">
						Recommended Interventions Breakdown
					</h3>
					<div className="grid grid-cols-1 gap-6">
						{data.recommendations.map((rec) => (
							<div
								key={rec.id}
								className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:border-emerald-300 transition-colors"
							>
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
									<div className="flex flex-col">
										<div className="flex items-center gap-1.5 text-xs text-gray-500 font-medium mb-1">
											<Leaf size={14} className="text-emerald-500" /> Est. CO₂
											Reduction
										</div>
									...
									<div className="flex flex-col">
										<div className="flex items-center gap-1.5 text-xs text-gray-500 font-medium mb-1">
											<Coins size={14} className="text-emerald-500" /> Estimated
											Cost
										</div>
									...
									<div className="flex flex-col">
										<div className="flex items-center gap-1.5 text-xs text-gray-500 font-medium mb-1">
											<Calendar size={14} className="text-gray-400" /> Payback
											Period
										</div>
										<div className="text-sm font-bold text-gray-900">
											{rec.payback}
