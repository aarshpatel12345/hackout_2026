	...
	return (
		<div className="absolute inset-0 z-50 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center rounded-2xl">
			...
			<h3 className="text-lg font-bold text-gray-900">
				AI Engine is analyzing your data...
			</h3>
			<p className="text-sm text-gray-500">
				Recalculating emissions and optimizing circular interventions.
			</p>
		</div>
	);
}
				...
				<div className="flex-1 bg-white rounded-2xl border border-gray-200 overflow-hidden relative shadow-sm flex min-h-[220px]">
					<div className="w-full lg:w-[60%] p-6 flex flex-col justify-center relative z-10 bg-gradient-to-r from-white via-white to-transparent">
						...
						<p className="text-gray-500 text-sm max-w-md leading-relaxed">
							Real-time carbon accounting, AI audit narrative, financial ROI
							calculations, and circular waste feedstock matching — all in one
							place.
						</p>
					</div>
				...
				<div className="w-full lg:w-72 bg-white rounded-2xl border border-emerald-100 shadow-sm p-5 flex flex-col justify-between border-t-4 border-t-emerald-400">
					<div>
						<div className="flex items-center gap-2 mb-3">
							...
							<h3 className="font-bold text-xs text-gray-500 tracking-wide uppercase">
								AI Analysis Engine
							</h3>
						</div>
						<p className="font-bold text-gray-900 text-base leading-tight mb-2">
							...
							<span className="text-xs font-normal text-gray-500">
								({data?.calculationEngine || "gemini-3.5-flash"})
							</span>
						</p>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
				...
				<div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm flex flex-col justify-between">
					<div>
						...
						<div className="flex items-end gap-2">
							<span className="text-4xl font-extrabold text-gray-900 tracking-tighter">
								~{Math.round(totalEmissions * 0.24).toLocaleString()}
							</span>
							<span className="text-sm font-semibold text-emerald-600 mb-1">
								tCO₂e/yr
							</span>
					...
					<div className="flex items-center gap-2 mt-4 text-[11px] font-medium text-emerald-700 bg-emerald-50 p-2 rounded-lg border border-emerald-100">
						...
						Equivalent to planting ~{Math.round(
							totalEmissions * 0.24 * 45,
						)}{" "}
						trees
					</div>
				</div>
				...
				<div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm flex flex-col justify-between">
					...
					<div className="h-8 mt-1 w-full relative">
						<div className="absolute -top-6 right-0 text-[10px] text-gray-400">
							by year-end
						</div>
					</div>
				...
				<div className="lg:col-span-5 bg-white rounded-2xl border border-gray-200 p-5 shadow-sm flex flex-col">
					...
					<div className="flex gap-4 justify-end mb-2 text-xs font-medium text-gray-600">
						<div className="flex items-center gap-1.5">
							<span className="w-2.5 h-2.5 rounded-sm bg-emerald-800"></span>{" "}
							Scope 1
						</div>
						<div className="flex items-center gap-1.5">
				...
				<div className="lg:col-span-4 bg-white rounded-2xl border border-gray-200 p-5 shadow-sm flex flex-col">
					...
					<div className="flex-1 flex items-center gap-4">
						...
						<div className="flex-1 space-y-3">
							<div className="text-[10px] text-gray-400 font-medium text-right mb-1 border-b border-gray-100 pb-1">
								%
							</div>
							...
							>
								<div className="flex items-center gap-3 text-sm font-medium">
									<action.icon
										size={16}
										className={
											action.primary ? "text-emerald-600" : "text-gray-400"
										}
									/>
									{action.name}
								</div>
								<ChevronRight
									size={16}
									className={
										action.primary ? "text-emerald-500" : "text-gray-300"
									}
								/>
							</button>
						))}
				...
				<div className="flex items-center justify-between mb-4">
					<div>
						...
						<p className="text-sm text-gray-500 mt-0.5">
							AI-prioritized recommendations based on your data, with estimated
							impact and ROI.
						</p>
					</div>
					<button
						onClick={() => navigate("/roi-calculator")}
						className="text-sm font-semibold text-gray-700 border border-gray-200 bg-white hover:bg-gray-50 px-4 py-2 rounded-lg flex items-center gap-2 transition-colors cursor-pointer shadow-sm"
					>
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					{recommendations.slice(0, 3).map((rec, idx) => (
						<div
							key={idx}
							onClick={() => navigate("/roi-calculator")}
							className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm hover:border-emerald-300 transition-colors cursor-pointer group flex flex-col justify-between"
						>
							<div>
								<div className="flex items-center justify-between mb-4">
									<span
							...
							<div className="grid grid-cols-3 gap-2 pt-4 border-t border-gray-100">
								<div>
									<div className="flex items-center gap-1 text-[10px] text-gray-500 font-medium mb-1">
										<Leaf size={12} className="text-emerald-500" /> CO₂ Red.
									</div>
								...
								<div>
									<div className="flex items-center gap-1 text-[10px] text-gray-500 font-medium mb-1">
										<Coins size={12} className="text-emerald-500" /> Est. Cost
									</div>
								...
								<div>
									<div className="flex items-center gap-1 text-[10px] text-gray-500 font-medium mb-1">
										<Calendar size={12} className="text-gray-400" /> Payback
									</div>
									<div className="text-[11px] font-bold text-gray-900">
										{rec.payback}
									</div>
					...
					{recommendations.length === 0 && (
						<div className="col-span-3 bg-gray-50 border border-gray-200 text-gray-500 p-8 rounded-2xl text-center">
							No recommendations generated yet. Run the AI analysis to get
							started.
						</div>
					)}
				</div>
